...
const GridCard = ({ grid, gridId, onDeleteGrid, onUpdateGrid, onDuplicateGrid }) => {
  const { _id, color, title } = grid;
  ...
  return (
    ...
    >
      ...
      {!isEdit && isActive ? (
                      ...
                      <Portal>
                        <Box zIndex={999} className="relative w-full h-full">
                          <PopoverContent boxSize="fit-content">
                            <ColorPicker
                              onClick={(color) => onUpdateGrid({ color })}
                            />
                          </PopoverContent>
                        </Box>
                      </Portal>
        ...
        <i className={classNames("bx-caret-down", { hidden: isEdit })}></i>
      )}
    </Link>
  );
};
